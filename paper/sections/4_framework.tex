...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    To evaluate both separation quality and partition balance, two common normalized metrics are used, the Normalized Cut, 
    $\operatorname{NCut} = \sum_{i=1}^k \frac{\operatorname{cut}(\mathcal{V}_i,\bar{\mathcal{V}}_i)}{\operatorname{vol}(\mathcal{V}_i)}$,
    % $\operatorname{NCut} = \sum_{i=1}^k \operatorname{cut}(\mathcal{V}_i, \bar{\mathcal{V}}_i)/\operatorname{vol}(\mathcal{V}_i)$
    where 
    $\operatorname{vol}(\mathcal{V}_i) = \sum_{v\in\mathcal{V}_i} \deg(v)$,
    and the Ratio Cut, $\operatorname{RCut} = \sum_{i=1}^k \frac{\operatorname{cut}(\mathcal{V}_i,\bar{\mathcal{V}}_i)}{|\mathcal{V}_i|}$. Both metrics penalize unbalanced partitions and favor cuts that correspond to sparse inter-partition connections in the graph. The provided benchmarking example scripts are:
